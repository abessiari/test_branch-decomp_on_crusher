#!/bin/bash
#SBATCH -A csc340
#SBATCH -J gto_tiny
#SBATCH -t 02:00:00
#SBATCH --mail-user=aessiari@lbl.gov
#SBATCH --mail-type=ALL
#SBATCH -N 1 

set -ex

EXTRA_OPTIONS="--augmentHierarchicalTree --computeVolumeBranchDecomposition"
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/opt/rocm-5.2.0/llvm/lib
executable=/ccs/home/aessiari/proj-shared/VTKM_MASTER/vtkm/build-debug/examples/contour_tree_distributed/ContourTree_Distributed
output_base_dir=/ccs/home/aessiari/proj-shared/VTKM_MASTER/results-debug

ls -l ${executable}
mkdir -p $output_base_dir
device=Kokkos

x=1
y=4

nRanks=$((x * y))
nNodes=1
dataset=gtopo_full_tiny_${x}x${y}
dataset_path=/ccs/home/aessiari/proj-shared/gtopo_data/gtopo/${dataset}/gtopo_full_tiny_part_%d_of_${nRanks}.txt

output_dir=${output_base_dir}/${dataset}_${nNodes}_nodes_${nRanks}_ranks_${device}_device_${SLURM_JOB_ID}_jobid
mkdir ${output_dir}
cd ${output_dir}

srun -n ${nRanks} -c 1 --gpus-per-task=1 --gpu-bind=closest ${executable} \
	--vtkm-log-level INFO --vtkm-device ${device} --forwardSummary --preSplitFiles \
       	$EXTRA_OPTIONS --numBlocks=${nRanks} ${dataset_path}
exit 0
